...
fn create_httpbin_https_check(path: &str, method: HttpMethod, expected_status: u16) -> HttpCheck {
    HttpCheck {
        ...
        auth: None,
        headers: None,
        assertions: Some(vec![HttpAssertion {
...
fn create_json_assertion(
    json_path: &str,
    predicate: AssertionPredicate,
    expected_value: AssertionValue,
) -> HttpAssertion {
    HttpAssertion {
        query: AssertionQuery::JsonPath {
...
fn create_header_assertion(
    header_name: &str,
    predicate: AssertionPredicate,
    expected_value: AssertionValue,
) -> HttpAssertion {
    HttpAssertion {
        query: AssertionQuery::Header {
...
fn test_httpbin_get_method_check() {
    ...
    assert!(evaluate_predicate(
        &status_assertion.predicate,
        &status_value,
        &status_assertion.value
    ));
...
fn test_httpbin_status_codes() {
    ...
    for (status_code, description) in test_cases {
        let check = create_httpbin_https_check(
...
fn test_httpbin_status_assertion_logic() {
    ...
    assert!(evaluate_predicate(
        &status_assertion_eq.predicate,
        &status_200,
        &status_assertion_eq.value
    ));
    assert!(evaluate_predicate(
        &status_assertion_gte.predicate,
        &status_200,
        &status_assertion_gte.value
    ));
    assert!(evaluate_predicate(
        &status_assertion_lt.predicate,
        &status_200,
        &status_assertion_lt.value
    ));
...
fn test_httpbin_basic_auth_check() {
    let mut check =
        create_httpbin_https_check("/basic-auth/testuser/testpass", HttpMethod::Get, 200);
...
fn test_httpbin_headers_configuration() {
    ...
    assert_eq!(
        headers_ref.get("User-Agent"),
        Some(&"UptimeMonitor/1.0".to_string())
    );
    assert_eq!(
        headers_ref.get("Accept"),
        Some(&"application/json".to_string())
    );
    assert_eq!(
        headers_ref.get("Authorization"),
...
fn test_httpbin_user_agent_check() {
    ...
    assert!(check.headers.is_some());
    assert_eq!(
        check.headers.as_ref().unwrap().get("User-Agent"),
        Some(&"UptimeMonitor/1.0".to_string())
    );
}
...
fn test_httpbin_json_path_assertions() {
    ...
    assert!(evaluate_predicate(
        &json_assertion.predicate,
        &url_value,
        &json_assertion.value
    ));
...
fn test_httpbin_header_assertions() {
    ...
    assert!(evaluate_predicate(
        &header_assertion.predicate,
        &json_content_type,
        &header_assertion.value
    ));
...
fn test_httpbin_body_regex_assertions() {
    ...
    assert_eq!(
        check.body_regex_check.as_ref().unwrap(),
        r"<title>.*</title>"
    );
}
...
fn test_httpbin_uuid_validation() {
    ...
    assert!(evaluate_predicate(
        &uuid_assertion.predicate,
        &valid_uuid,
        &uuid_assertion.value
    ));
...
fn test_httpbin_ipv6_validation() {
    ...
    assert!(evaluate_predicate(
        &ipv6_assertion.predicate,
        &valid_ipv6,
        &ipv6_assertion.value
    ));
...
fn test_httpbin_iso_date_validation() {
    ...
    assert!(evaluate_predicate(
        &date_assertion.predicate,
        &valid_iso_date,
        &date_assertion.value
    ));
...
fn test_httpbin_certificate_assertions() {
    ...
    let cert_subject_assertion = HttpAssertion {
        query: AssertionQuery::Certificate {
            field: CertificateField::Subject,
        },
    ...
    let cert_algorithm_assertion = HttpAssertion {
        query: AssertionQuery::Certificate {
            field: CertificateField::Algorithm,
        },
        predicate: AssertionPredicate::Exists,
        value: AssertionValue::String("".to_string()),
    };
    ...
    match (
        &cert_subject_assertion.query,
        &cert_subject_assertion.predicate,
    ) {
        (
            AssertionQuery::Certificate {
                field: CertificateField::Subject,
            },
            AssertionPredicate::Contains,
        ) => {
    ...
    match (
        &cert_issuer_assertion.query,
        &cert_issuer_assertion.predicate,
    ) {
        (
            AssertionQuery::Certificate {
                field: CertificateField::Issuer,
            },
            AssertionPredicate::Exists,
        ) => {
            // Test passed - correct query and predicate combination
        }
    ...
    match (
        &cert_algorithm_assertion.query,
        &cert_algorithm_assertion.predicate,
    ) {
        (
            AssertionQuery::Certificate {
                field: CertificateField::Algorithm,
...
fn test_httpbin_complex_assertions_combination() {
    let mut check =
        create_httpbin_https_check("/get?param1=value1&param2=value2", HttpMethod::Get, 200);
    ...
    let config = AppConfig {
        ...
        hosts: vec![
            HostConfig {
                ...
                checks: vec![Check::Http(create_httpbin_https_check(
                    "/status/200",
                    HttpMethod::Get,
                    200,
                ))],
            ...
            HostConfig {
                ...
                checks: vec![Check::Http({
                    let mut check =
                        create_httpbin_https_check("/basic-auth/user/pass", HttpMethod::Get, 200);
                    check.auth = Some(AuthConfig::Basic {
