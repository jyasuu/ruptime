...
async fn make_httpbin_request(
    check: &HttpCheck,
) -> Result<(reqwest::StatusCode, String, reqwest::header::HeaderMap), reqwest::Error> {
...
async fn test_httpbin_status_codes_real() {
    ...
    for status_code in test_cases {
        ...
        assert!(
            result.is_ok(),
            "Failed to make request for status {}: {:?}",
            status_code,
            result.err()
        );
...
async fn test_httpbin_bearer_auth_real() {
    ...
    assert!(
        result.is_ok(),
        "Failed to make bearer auth request: {:?}",
        result.err()
    );
...
async fn test_httpbin_headers_real() {
    ...
    assert!(
        result.is_ok(),
        "Failed to make headers request: {:?}",
        result.err()
    );
...
async fn test_httpbin_user_agent_real() {
    ...
    assert!(
        result.is_ok(),
        "Failed to make user-agent request: {:?}",
        result.err()
    );
...
async fn test_httpbin_redirect_real() {
    ...
    assert!(
        response.is_ok(),
        "Failed to make redirect request: {:?}",
        response.err()
    );
...
async fn test_httpbin_delay_real() {
    ...
    assert!(
        duration.as_secs() >= 1,
        "Request should have taken at least 1 second, took: {:?}",
        duration
    );
    assert!(
        duration.as_secs() < 5,
...
async fn test_httpbin_assertions_with_real_data() {
    ...
    let number_value = Value::String(
        json_response["args"]["number"]
            .as_str()
            .unwrap()
            .to_string(),
    );
...
async fn test_httpbin_content_types_real() {
    ...
    for (path, expected_content_type) in content_type_cases {
        ...
        assert!(
            ...
            "Content-Type header missing for {}",
            path
        );

        let content_type_str = content_type.unwrap().to_str().unwrap();
        assert!(
            content_type_str.contains(expected_content_type),
        ...
        println!(
            "✓ Content-Type test for {} successful: {}",
            path, content_type_str
        );
    }
}
