use crate::config::{
    ElasticsearchCheck, KafkaCheck, MongoDBCheck, MySQLCheck, PostgresCheck, PostgresSslMode,
...
pub async fn check_rabbitmq(address: &str, config: &RabbitMQCheck) -> ServiceCheckResult {
    ...
    let url = format!(
        "{}://{}:{}/api/overview",
        protocol, address, management_port
    );
    ...
    .await
    {
        ...
        Ok(Err(e)) => ServiceCheckResult {
            status: CheckStatus::Unhealthy(format!(
                "RabbitMQ Management API request failed: {}",
                e
            )),
            response_time_ms: start_time.elapsed().as_millis(),
            service_info: None,
        ...
        Ok(Ok(mut conn)) => match conn.query_first::<String, _>("SELECT VERSION()").await {
            ...
            Ok(None) => ServiceCheckResult {
                status: CheckStatus::Unhealthy(
                    "MySQL VERSION() query returned no result".to_string(),
                ),
                response_time_ms: start_time.elapsed().as_millis(),
                service_info: None,
            },
...
pub async fn check_mongodb(address: &str, config: &MongoDBCheck) -> ServiceCheckResult {
    ...
    let mut uri = if config.use_ssl {
        format!(
            "mongodb://{}:{}/{}?ssl=true",
            address, config.port, config.database
        )
    } else {
    ...
    if let (Some(username), Some(password)) = (&config.username, &config.password) {
        uri = uri.replace(
            "mongodb://",
            &format!("mongodb://{}:{}@", username, password),
        );
    }
